...
model.load_state_dict(torch.load("best_model4_real.pth", map_location=device))
model.to(device)
model.eval()
...
while running:
    ...
    sd.wait()
    #test###########
    audio = wavfile.read("dronesound_hrtf_snippets_real/az120_el0_snippet010.wav")
    audio=audio[1]
    ###############
    # t0 = stamp("Recorded Audio", t0)
    # max_first_20 = np.max(audio[:20, :])
    # max_last_20  = np.max(audio[-20:, :])
    # fade_in = np.linspace(0, audio.max(), fade_samples)
    # fade_out = np.linspace(audio.max(), 0, fade_samples)
